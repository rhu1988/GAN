...
train_dataset = image_dataset_from_directory(img_dir_path,
                                             shuffle=True,
                                             batch_size=BATCH_SIZE,
                                             image_size=(img_height,img_width))
# unzip image dataset from train_dataset
img_dataset=train_dataset.map(lambda img,label: img)
...
def train_display_image(model, epoch):
    ...
    for i in range(predictions.shape[0]):
        plt.subplot(4, 4, i+1)
        plt.imshow(tf.reshape(predictions[i], [255, 255]), cmap='Colormap')
        plt.axis("off")
...
def sample(size, cat=-1, c1=None, c2=None):
    z = tfd.Uniform(low=-1.0, high=1.0).sample([size, 268])
    ...
    if cat >= 0:
        ...
        z_cat = tf.one_hot(z_cat, 7)
    else:
        z_cat = tfd.Categorical(probs=tf.ones([7])*0.1).sample([size,])
        z_cat = tf.one_hot(z_cat, 7)
